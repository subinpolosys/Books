package pages;
	...
	public class CreateLadedCostPIPage {
		private final WebDriver driver;
	    private final WebDriverWait wait;
	    public CreateLadedCostPIPage(WebDriver driver) {
	        this.driver = driver;
...
//	    private final By termsField = By.id("terms_and_conditions");
	 ...
	 // ──────────────── Transaction level Discount ────────────────
	    ...
	    //______________Landed Cost_________________________
	    private final By addLandedcostBtnField=By.xpath("//button[text()='Add Landed Cost']");
	    private final By landedcostListField=By.xpath("(//input[@placeholder='Type or click to add items'])[2]");
...
//		        else if ("At Line Item Level".equalsIgnoreCase(discountLevel)) {
	    ...
	    public void addItems(
	        ...
	        for (int i = 0; i < itemNames.length; i++) {
	        	if(!Utilities.isNotEmpty(landed[i])) 
	        	{	        		
				...
				WebElement addLC  = driver.findElement(addLandedcostBtnField);
			    ls.executeScript("arguments[0].scrollIntoView();",addLC);  
				driver.findElement(addLandedcostBtnField).click();  //#####  
				Thread.sleep(500);	
	        	 //driver.findElement(addLandedcostBtnField).click();
	        	  wait.until(ExpectedConditions.elementToBeClickable(landedcostListField)).click();
		            driver.findElement(landedcostListField).sendKeys(itemNames[i]);
		            Thread.sleep(500);
		            // Select item dynamically
		            By selectItem = By.xpath(String.format(itemSelectField, 1));
		            wait.until(ExpectedConditions.elementToBeClickable(selectItem)).click();
		            Thread.sleep(200);
		            // Fill item quantity                                        (//table/tbody)[2]/tr[1]
		            WebElement qtyField = driver.findElement(By.xpath("(//table/tbody)[2]/tr["+l+"]/td[5]//input"));  //(//tbody/tr["+i+"])[2]/td[5]//input
		            qtyField.clear();
		            qtyField.sendKeys(itemQtys[i]);
		            //WaitUtils.waitForUi(driver);
		            if(discLevel!=0) {
		            	if (discType[i] != null && !discType[i].trim().isEmpty() && discount[i] != null && !discount[i].trim().isEmpty()) {
		            		if("%".equalsIgnoreCase(discType[i])) {
			            		...
			            		WebElement discountField=driver.findElement(By.xpath("(//table/tbody)[2]/tr["+l+"]/td[7]/div[1]/input"));
			            		discountField.clear();
			            		discountField.sendKeys(discount[i]);
		            		}
			            	else {
			            		By discDropdown = By.xpath("(//table/tbody)[2]/tr["+l+"]/td[7]/div[1]/div");
			            		WebElement dropdown = wait.until(ExpectedConditions.elementToBeClickable(discDropdown));
			            		dropdown.click();
			            		// Wait for dropdown options globally
			            		By optionLocator = By.xpath("//ul/li[2]");
			            		...
			            		option.click();
			            		// Now enter value
			            		By discountFieldLocator = By.xpath("(//table/tbody)[2]/tr["+l+"]/td[7]/div[1]/input");
			            		WebElement discountField = wait.until(ExpectedConditions.visibilityOfElementLocated(discountFieldLocator));
			            		discountField.clear();
			            		discountField.sendKeys(discount[i]);
			            	}
